...
Differences from ``DIV2KPSFDataset`` (RGB version):
    ...
    - LQ output is 1ch (no XY) or 3ch (1 gray + 2 XY).
"""
...
class DIV2KPSFGrayDataset(data.Dataset):
    ...
    Args:
        opt (dict): Config dict containing:
            ...
            psf_dir (str): Directory with .npz PSF kernel files.
            ...
            psf_edge_threshold (int): Edge PSF filter threshold for validation.
            use_xy (bool): Whether to append XY coordinate channels.
    """
    ...
    def __init__(self, opt):
        ...
        if self.psf_edge_threshold > 0 and not self.is_train:
            ...
            print(f'  [DIV2KPSFGrayDataset] Edge PSF pool: {len(self.psf_edge_pool)} / '
        ...
        print(f'  [DIV2KPSFGrayDataset] {n_paths} images | {len(self.psf_kernels)} PSF kernels | '
    ...
    def __getitem__(self, index):
        ...
        if self.use_xy:
            ...
            # Stack LQ (1ch gray) + XY (2ch) → (H, W, 3)
